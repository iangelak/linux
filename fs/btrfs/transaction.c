				     ...
				     unsigned int type)
{
	...
	rwsem_acquire_read(&fs_info->btrfs_trans_num_writers_map, 0, 0, _THIS_IP_);
	spin_lock(&fs_info->trans_lock);
loop:
	/* The file system has been taken offline. No new transactions. */
	if (BTRFS_FS_ERROR(fs_info)) {
		spin_unlock(&fs_info->trans_lock);
		rwsem_release(&fs_info->btrfs_trans_num_writers_map, _THIS_IP_);
		return -EROFS;
	}
				   ...
				   int throttle)
{
	...
	rwsem_release(&info->btrfs_trans_num_writers_map, _THIS_IP_);
...
static void cleanup_transaction(struct btrfs_trans_handle *trans, int err)
{
	...
	if (cur_trans == fs_info->running_transaction) {
		...
		btrfs_might_wait_for_num_writers(fs_info);
		spin_unlock(&fs_info->trans_lock);
